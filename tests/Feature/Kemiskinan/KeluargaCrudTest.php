...
it('menyimpan pendataan keluarga baru sebagai draft beserta data turunannya', function () {
    $user = User::factory()->create();
    ...
    expect($keluarga->status_data)->toBe('draft');
...
it('mengirim pendataan langsung untuk verifikasi dan mencatat riwayat', function () {
    ...
    $this->actingAs($user)->post(route('kemiskinan.store'), payloadPendataan($kecamatan, $desa, ['action' => 'kirim']));

    $keluarga = Keluarga::first();

    expect($keluarga->status_data)->toBe('dikirim');
    expect($keluarga->riwayatVerifikasi)->toHaveCount(1);
    expect($keluarga->riwayatVerifikasi->first()->status)->toBe('dikirim');
});
...
it('menyimpan draft hanya dengan identitas keluarga tanpa anggota maupun kondisi rumah', function () {
    $user = User::factory()->create();
    $kecamatan = Kecamatan::factory()->create();
    $desa = DesaKelurahan::factory()->create(['kecamatan_id' => $kecamatan->id]);

    $payload = payloadPendataan($kecamatan, $desa);
    unset($payload['anggota'], $payload['kondisi_rumah']);
    ...
    expect($keluarga)->not->toBeNull();
    expect($keluarga->status_data)->toBe('draft');
    expect($keluarga->jumlah_anggota_keluarga)->toBe(0);
    expect($keluarga->anggotaKeluarga)->toHaveCount(0);
    expect($keluarga->kondisiRumah)->toBeNull();
});

it('menolak pengiriman verifikasi tanpa anggota keluarga', function () {
    $user = User::factory()->create();
    $kecamatan = Kecamatan::factory()->create();
    $desa = DesaKelurahan::factory()->create(['kecamatan_id' => $kecamatan->id]);

    $payload = payloadPendataan($kecamatan, $desa, ['action' => 'kirim']);
...
it('menolak pengiriman verifikasi tanpa kondisi rumah', function () {
    $user = User::factory()->create();
    $kecamatan = Kecamatan::factory()->create();
    $desa = DesaKelurahan::factory()->create(['kecamatan_id' => $kecamatan->id]);
